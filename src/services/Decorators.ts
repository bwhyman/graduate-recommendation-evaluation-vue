 ...
 */
export function StoreCache(dataR: ShallowRef<any>, replace = false) {
  return (_: any, __: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async (...args: any[]) => {
      ...
      // 异步执行目标方法并将结果置于store
      const r = (await originalMethod.apply(descriptor, args)) as ShallowRef
      return (dataR.value = r.value) && dataR
    }
    return descriptor
  }
}
 ...
 */
export function StoreMapCache(
  dataR: ShallowRef<Map<any, ShallowRef>>,
  { replace = false, indexs = [] as number[] } = {}
) {
  return (_: any, __: string, descriptor: PropertyDescriptor) => {
    const originalMethod = descriptor.value
    descriptor.value = async (...args: any[]) => {
...
export function StoreClearObject(...clears: any[]) {
  return (_: any, __: string, descriptor: PropertyDescriptor) => {
    const originalMethod = descriptor.value
    descriptor.value = (...args: any[]) => {
      for (const clear of clears) {
...
export function StoreClearMap(...clears: Map<any, any>[]) {
  return (_: any, __: string, descriptor: PropertyDescriptor) => {
    const originalMethod = descriptor.value
    descriptor.value = (...args: any[]) => {
      for (const clear of clears) {
